...
def folderTree(folders, indent=0):
    ...
    for folder in folders:
        print(
            "{0}{1}. {2} ({3})".format(prefix, i, folder.Name, folder.DefaultItemType)
        )
...
def printCalendar(folder):
    ...
    for appointmentItem in restrictedItems:
        ...
        print("{0} - {1}: {2} ({3})".format(startDate, endDate, subject, organizer))
...
if __name__ == "__main__":
    ...
    namespace = (
        Outlook.Session
    )  # identical to GetNameSpace("MAPI") (starting with Outlook 98)
    ...
    print("\nShared calendar")
    print("---------------")
    sharedCalendar = findFolder(
        namespace.Folders, ["Internet Calendars", "youremail@email.com"]
    )
    printCalendar(sharedCalendar)
